        ...
        for l1 in range(1):
            ...
            #j=np.random.uniform(1,1000)
                if(b>a):
                    Fo=getFo(a,b,l)
                    Bo=getBo(j[m],a,Fo)
                    z=np.linspace(-3*l/2,3*l/2,numOfSamples)
                    F=getF(z,a,l,b)
...
np.savetxt(dataFileName,supListArray,delimiter=' ')
out_suplist=np.array(out_suplist)
dictFileName="out_prac_"+fileSuffix
np.savetxt(dictFileName,out_suplist,delimiter=' ')
